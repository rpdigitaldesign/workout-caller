...
- **This project lives inside a Dropbox-synced folder — `node_modules` and `.next` are excluded from Dropbox sync** (via `xattr -w com.dropbox.ignored 1 <folder>`, Dropbox's supported per-folder ignore mechanism), not just from git. Both directories are rewritten continuously by `npm install` and the running dev server; letting Dropbox sync them at the same time causes write conflicts (you'll see "conflicted copy" files appear inside `.next/dev`) that can corrupt the dev server's build manifest and produce confusing Turbopack errors like `module factory is not available`. If you ever move this project to a new machine/Dropbox folder, re-apply the ignore flag: `xattr -w com.dropbox.ignored 1 node_modules && xattr -w com.dropbox.ignored 1 .next`.
- **After installing any new dependency, fully restart `npm run dev`** rather than relying on hot-reload to pick it up — Turbopack (like webpack before it) doesn't always register a brand-new `node_modules` package cleanly into an already-running dev server's module graph, which can also produce a `module factory is not available` error for that package's first import.
